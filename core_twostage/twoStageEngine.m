...
% @ 2009 Byron Yu -- byronyu@stanford.edu
  ...
  for k = 1:length(kernSDList)
    ...
    elseif isequal(typ, 'pca')
      % replaced deprecated 'princomp' with 'pca'
      %[pcDirs, pcScores] = pca(Y');
      [pcDirs, pcScores, latent, tsquared, explained, mu] = pca(Y');
%       plotPCAVarianceVSDim(explained);
      kern(k).seqTrain = segmentByTrial(kern(k).seqTrain,... 
                            pcScores(:,1:xDim)', 'xpost');  
           
      kern(k).estParams.L = pcDirs(:, 1:xDim);
      kern(k).estParams.latent = latent;
      kern(k).estParams.explained = explained;
      kern(k).estParams.d = mean(Y, 2);    
      
    % author: sapan@cs.wisc.edu
    % 'lda': linear discriminant analysis
    elseif isequal(typ, 'lda')
      % 
      [coeff, score, latent, explained] = linearDiscriminantAnalysis(kern(k).seqTrain, Y', classes);
      kern(k).seqTrain = segmentByTrial(kern(k).seqTrain,... 
                              score(:,1:xDim)', 'xpost');  
      kern(k).estParams.L = coeff(:, 1:xDim);
      kern(k).estParams.d = mean(Y, 2);    
    end    
